...
class ir_attachment(models.Model):
    ...
    def find_or_create_root_directory(self, client):
        ...
        # check that it's still exist in onedrevi
        try:
            client.item(id=res_id).get()
        except:
            res_id = False
        if not res_id:
    ...
    def find_or_create_model_directory(self, client, root_id):
        ...
        model_name = self.res_model and self.env[self.res_model]._description or self._description
        ...
        if model:
            try:
                model_directory = client.item(id=model.onedrive_id)
                model_directory.get()  # check that exist
            except:
                model_directory = False

        if not model_directory:
            model_directory = mkdir(client, drive, root_id, model_name)
            values = {
                'name': model_name,
                'onedrive_id': model_directory,
            }
    ...
    def find_or_create_object_directory(self, client, model_dir_id):
        ...
        object_model = self.res_model or self._name
        object_id = self.res_model and self.res_id or self.id
        obj = self.env['onedrive.object'].search(
            [('name', '=', object_model), ('res_id', '=', object_id)],
            limit=1,
            order="id desc",
        )
        if obj:
            try:
                object_directory = client.item(id=obj.onedrive_id)
                object_directory.get()  # check that exist
            except:
                object_directory = False

        if not object_directory:
            rec = self.env[object_model].browse(object_id)
            ...
            object_directory = client.item(id=res)
    ...
    def send_to_cloud(self, client=False):
        ...
        try:
            ...
            download_url = False
            _logger.warning("Got next problem white try sync attachment with ID {}:\n%s".format(self.id), error)
        ...
        with api.Environment.manage():
            with registry(self._cr.dbname).cursor() as new_cr:
                ...
                for obj in ObjectMap.search([]):
                    try:
                        client.item(id=obj.onedrive_id).get()
                        ...
                        for oid in to_create:
                            try:
                                ...
                                _logger.debug(
                                    "Item deleted from onedrive server: ID {}".format(oid),
                                )
                        ...
                        _logger.debug(
                            "Object deleted from onedrive server: ID {} - {}".format(obj.onedrive_id, obj.name),
                        )
